...
if (mayStatus.bmi >= 35 || mayStatus.isVip) {
  console.log("小美可以打九折！");
} else {
  console.log("小美不能打九折！");
}
...
/*
  ...
  1.月收入 4 萬元(含)以下。
  2.歷史申請紀錄不得超過 4 次。
  請幫小明判斷他是否能申請學貸緩繳通過，如果能夠通過，
  使用陣列方法將他的 name、salary 放入 successList 內的 name、salary 裡。
*/
// 題目二
let successList = {
  name: ["小花", "小美", "小龜", "小茵"],
...
// 小提示 : 透過陣列方法 .length 判斷歷史申請年份 "applyHistory" 是否小於或等於 4 次。
if (mingStatus.salary <= 4 && mingStatus.applyHistory.length <= 4) {
  ...
  successList.name.push(mingStatus.name);
  successList.salary.push(mingStatus.salary);
  console.log("小明申請通過！");
